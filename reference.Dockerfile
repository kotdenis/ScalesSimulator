...
RUN dotnet restore Scales.ReferenceBook.Api.csproj
COPY . .

RUN dotnet build ./Scales.ReferenceBook.Api/Scales.ReferenceBook.Api.csproj -c Release -o /app

FROM build AS publish
RUN dotnet publish ./Scales.ReferenceBook.Api/Scales.ReferenceBook.Api.csproj -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Scales.ReferenceBook.Api.dll"]